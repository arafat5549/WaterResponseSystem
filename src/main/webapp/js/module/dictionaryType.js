...
{field:"code",			title:"编码",	align:"center",		width:150},
...
function addInfo(){
    EasyUI.occDialog('新增品牌', 320, 'auto','icon-add', 'page/dictionaryType/edit.jsp');
    $('#fm').form('clear');
    method = 'POST';
}
function updateInfo(){
	...
	if(row){
        EasyUI.occDialog('更新品牌', 320,'auto', 'icon-edit', 'page/dictionaryType/edit.jsp', function () {
            $('#fm').form('load', row);
        });
        method = "PUT";
	}else{
...
function commitInfo(){
	...
	if(flag){
		...
		data['_method'] = method;
		myForm.ajaxRequest(basePath+'api/dictionaryTypes',data,function (r) {
            if(r.result){
...
function deleteInfo(){
	...
	if(row){
		$.messager.confirm('提示信息' , '确认删除品牌【'+row.name+'】吗？', function(r){
			if(r){
				myForm.ajaxRequest(basePath+'api/dictionaryTypes',{_method:'DELETE',id:row.id},function (r) {
                    if(r.result){
                        $.messager.alert('信息',r.msg);
                        grid.datagrid('load', {});
                        grid.datagrid('unselectAll');
...
$(function(){
	grid = EasyUI.dataGrid('grid',gridColumns,basePath+"api/dictionaryTypes");
});