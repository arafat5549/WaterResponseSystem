...
function addInfo() {
    EasyUI.occDialog('新增部门', 320,'auto', 'icon-add', 'page/category/edit.jsp');
    $('#fm').form('clear');
    method = 'POST';
}
...
function updateInfo() {
    ...
    if (node) {
        EasyUI.occDialog('更新部门', 320,'auto', 'icon-edit', 'page/category/edit.jsp', function () {
            $('#fm').form('load', node);
        });
    } else {
        $.messager.show({
...
function commitInfo() {
    ...
    if (flag) {
        ...
        data['_method'] = method;
        myForm.ajaxRequest(basePath + 'api/categorys', data, function (r) {
            if (r.result) {
                ...
                $("#addParent").combotree("reload", basePath + 'api/categorys');
                $("#queryParent").combotree("reload", basePath + 'api/categorys');
            } else {
...
function deleteInfo() {
    ...
    if (node) {
        $.messager.confirm('提示信息', '确认删除部门【' + node.text + '】吗？', function (r) {
            if (r) {
                myForm.ajaxRequest(basePath + 'api/categorys', {_method: 'DELETE', id: node.id}, function (r) {
                    if (r.result) {
                        $.messager.alert('信息', r.msg);
                        treeGrid.treegrid('load', {});
                        treeGrid.treegrid('unselectAll');
...
$(function () {
    treeGrid = EasyUI.treeGrid('treeGrid', treeGridFrozenColumns, treeGridColumns, basePath + 'api/categorys');
});